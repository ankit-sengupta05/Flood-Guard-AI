...
import { Activity, ArrowRight, ChevronRight, Droplets, Waves, X } from 'lucide-react';
import type { Dam } from '../types';
import { HERO_DAM, SECONDARY_DAM } from '../data/demoData';

interface HomeProps { onStartSimulation: (dam: RegistryDam) => void; onViewArchitecture: () => void; }
...
export const HomeLanding: React.FC<HomeProps> = ({ onStartSimulation, onViewArchitecture }) => {
  const [dams, setDams] = useState<RegistryDam[]>(FALLBACK_DAMS);
  const [selected, setSelected] = useState<RegistryDam>(FALLBACK_DAMS[0]);
  const [focusedDam, setFocusedDam] = useState<RegistryDam>();
